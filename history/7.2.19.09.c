...
static void gpio_callback_func(pin_t pin, uintptr_t param)
{
    ...
    // 1. 如果是初始状态、红外报警状态、或者状态b(及其结束状态)，按下后一律进入状态1(a)
...
static void pir_interrupt_handler(pin_t pin, uintptr_t param)
{
    ...
    if (g_servoRun == 1 || g_servoRun == 2) {
        ...
        printf("PIR Triggered! Servo emergency stop.\r\n");
    }
}
...
void UpdateOledDisplay(int state, float temp, float humi)
{
    ...
    if (state == 1 || state == -1) {
        ssd1306_DrawString("Servo: State A (CW)", Font_7x10, 1); 
    } else if (state == 2 || state == -2) {
        ssd1306_DrawString("Servo: State B (CCW)", Font_7x10, 1);
    } else if (state == 3) {
...
void mainTask(void)
{
    ...
    int last_state = -99; // 确保第一次进入循环时能刷新灯
    ...
    while (1)
    {
        ...
        /* ADC 电压检测逻辑 (约每 100ms * 30 = 3000ms 检测一次) */
        if (tick_count % 30 == 0)
        {
            if (adc_port_read(CONFIG_ADC_CHANNEL, &adc_voltage) == ERRCODE_SUCC) {
                ...
                // 条件判断：电压低于1300mv 且 黄灯处于常亮状态（状态b转完之后的挂起态-2）
                if (adc_voltage < 1300 && g_servoRun == -2) {
                    printf("[ADC TRIGGER] Voltage < 1300mV & Yellow LED On. Switching to State A!\r\n");
                    g_servoRun = 1;        // 切换到状态a
                    g_servoActionDone = 0; // 清除动作标记，允许舵机开始旋转
                }
            } else {
        ...
        if (g_servoRun == 1 && !g_servoActionDone)
        {
            ...
            for(int i = 0; i < 12; i++) {
                if (g_servoRun != 1) break; // 允许中途被红外中断强制退出
                EngineTurnRight(); 
            ...
            if (g_servoRun == 1) {
                g_servoActionDone = 1; 
                g_servoRun = -1;  // 转完后进入状态a对应的静止挂起态，灯光维持绿色
            }
        }
        else if (g_servoRun == 2 && !g_servoActionDone)
        {
            ...
            for(int i = 0; i < 12; i++) {
                if (g_servoRun != 2) break; // 允许中途被红外中断强制退出
                EngineTurnLeft(); 
                uapi_systick_delay_ms(50);
            }
            ...
            if (g_servoRun == 2) {
                g_servoActionDone = 1; 
                g_servoRun = -2;  // 转完后进入状态b对应的静止挂起态，灯光维持黄色
            }
        }
        else
        {
            // 降低空载延时为 100ms，方便更精确地控制计数器 tick_count (30 * 100ms = 3000ms)
            uapi_systick_delay_ms(100);
        }
