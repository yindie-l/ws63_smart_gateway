...
/* 广播名称 */
static uint8_t sle_local_name[NAME_MAX_LENGTH] = {'S', 'L', 'E', '_', 'W', 'i', 'n', 'd', 'o', 'w', '_',
                                                  'S', 'e', 'r', 'v', 'e', 'r', '\0'};
...
static void example_sle_set_addr(void)
{
    ...
    sle_addr.type = 0;
    PRINT("[SLE Adv] configured addr=%02x:%02x:%02x:%02x:%02x:%02x\r\n",
...
static uint8_t g_local_device_name[] = {'S', 'L', 'E', '_', 'W', 'i', 'n', 'd', 'o', 'w', '_', 'S', 'e', 'r', 'v', 'e', 'r'};
...
static errcode_t example_sle_set_default_announce_data(void)
{
    ...
    if (ret == ERRCODE_SUCC) {
        ...
        PRINT("[SLE Adv] set announce data failed, ret=0x%x\r\n", ret);
    }
    return ret;
}
...
errcode_t example_sle_server_adv_init(void)
{
    ...
    if (ret != ERRCODE_SUCC) {
        PRINT("[SLE Adv] start announce failed, ret=0x%x\r\n", ret);
        return ret;
    }
